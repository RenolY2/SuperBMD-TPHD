            ...
            foreach (Mesh mesh in scene.Meshes)
            {
                ...
                else
                {
                    ...
                    if (mesh.Name.Contains("_NoWeights_"))
                    {
                        ...
                        if (!boneNames.ContainsKey(key))
                        {
                            key = "skeleton_root_" + result[1];
                            if (!boneNames.ContainsKey(key))
                            {
                                throw new Exception(String.Format("No such bone name: {0}", result[1]));
                            }
                        }
                        jointindex = boneNames[key];
                        transformVerts = true;
                    }

                    partialWeight.WeightTypeCheck.Add(false);
                    partialWeight.Indices.Add(jointindex);
                    meshShape.ProcessVerticesWithoutWeights(mesh, vertData, envelopes, jointindex, partialWeight.Indices.Count - 1, transformVerts);
